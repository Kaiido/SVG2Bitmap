function SVG2Bitmap(svg, receiver, params) {
  ...
  var frame; // in case user passed a framed svg
  ...
  if(svg.nodeName === "OBJECT" || svg.nodeName === "IFRAME") {
    ...
    if(svg.contentDocument.readyState !== 'complete') {
      svg.addEventListener('load', frameLoadHandler);
      return;
    }
  ...
  function frameLoadHandler() {
    // remove our handler
    this.removeEventListener('load', frameLoadHandler);
    // recall the function
  ...
  // start by parsing the applied styles
  parseStyles();
  ...
  function exportDoc() {
    ...
    // And, it can create bad things with absolutely positioned elements.
  ...
  function parseImages() {
    ...
    function toDataURL(image, original) {
      var img = new Image();
    ...
    function parseFromUrl(url, element) {
  ...
  // svg's preserveAspectRatio rules to canvas2d 'drawImage' parameters
  function preserveAspectRatio(source, destination, userString) {
    var srcWidth = source.width,
      srcHeight = source.height,
      destinationW = destination.width,
      destinationH = destination.height;
    // we should keep the whole source
    function aRMeet(args) {
      var srcRatio = (srcHeight / srcWidth),
        destRatio = (destinationH / destinationW),
        resultWidth = destRatio > srcRatio ? destinationW : destinationH / srcRatio,
        resultHeight = destRatio > srcRatio ? destinationW * srcRatio : destinationH;

      function getPos(arg, res, dest) {
    ...
    function aRSlice(args) {
      ...
      function getPos(arg, res, dest, src) {
        switch(arg) {
          case 'Min':
            return 0;
          case 'Mid':
    ...
    if(!userString) {
      ...
      var args = userString.trim()
        .split(' '),
        minMidMax = args[0].replace('x', '')
        .split('Y');
  ...
  function parseXlinks(css) {
    ...
    var changeImagesHref = function(elem, base) {
      ...
      for(var i = 0; i < images.length; i++) {
        var href = images[i].getAttributeNS(xlinkNS, 'href');
        var newHref = tester.URL(href, base)
          .href;
        if(href !== newHref) {
    ...
    var append_doc = function(iri, doc) {
      ...
      var original_filename = a.href.substring(a.href.lastIndexOf('/') + 1)
        .replace(a.hash, '');
